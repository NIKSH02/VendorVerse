...
ReviewSchema.pre('validate', function(next) {
    if ((this.orderId && this.sampleId) || (!this.orderId && !this.sampleId)) {
        next(new Error('Review must have either orderId or sampleId, but not both'));
    } else {
        next();
    }
});
...
ReviewSchema.post('save', async function() {
    ...
    const reviews = await this.constructor.find({ toUserId: this.toUserId, isVerified: true });
