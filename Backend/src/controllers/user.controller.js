...
const { generateOTP, generateOTPExpiry, verifyOTP, canResendOTP } = require('../utils/otpGenerator');
const { sendSignupOTP, sendSigninOTPEmail } = require('../utils/emailService');


const registerUser = asynchandler(async (req, res) => { 
    const { username, email, password, fullname } = req.body;
    console.log('Registration request:', { username, email, password: password ? 'PROVIDED' : 'MISSING', fullname });
    ...
    try {
        ...
        console.log('Found existing user:', existedUser ? {
            ...
            isFullyRegistered: existedUser.isEmailVerified && 
        ...
        if (existedUser) {
            ...
            const isFullyRegistered = existedUser.isEmailVerified && 
                existedUser.password && 
                !existedUser.password.startsWith('temp_password_') &&
                !existedUser.username.startsWith('temp_');
            ...
            if (isFullyRegistered) {
                ...
                existedUser.avatar = `https://ui-avatars.com/api/?name=${encodeURIComponent(fullname || username)}&background=6366f1&color=ffffff&size=200`;
                
                await existedUser.save();
                
                // Return user without password
                const updatedUser = await User.findById(existedUser._id).select('-password -refresh_token');
        ...
        if (error.code === 11000) {
            ...
            throw new apiError(409, `A user with this ${field} (${value}) already exists. Please choose a different ${field}.`);
        }
...
const sendEmailVerificationOTP = asynchandler(async (req, res) => {
    ...
    if (existingUser && existingUser.isEmailVerified && 
        existingUser.password && !existingUser.password.startsWith('temp_password_')) {
    ...
    console.log('Existing user found:', existingUser ? 'Yes (not fully registered)' : 'No');

    // Check if we can send OTP (rate limiting)
    if (existingUser && !canResendOTP(existingUser.emailVerificationOTPLastSent)) {
    ...
    try {
        ...
        if (existingUser) {
            ...
            const tempUsername = `temp_${baseUsername}_${timestamp}_${randomSuffix}`.toLowerCase();
...
const verifyEmailOTP = asynchandler(async (req, res) => {
    ...
    // Verify OTP
    const isValid = verifyOTP(otp, user.emailVerificationOTP, user.emailVerificationOTPExpiry);
    ...
    return res.status(200).json(
        new ApiResponse(200, 'Email verified successfully. Please complete your registration.', { 
            email, 
            isEmailVerified: true 
        })
    );
...
const resendEmailVerificationOTP = asynchandler(async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json(
            new ApiResponse(200, 'New OTP sent successfully', { email })
        );
    } catch (error) {
...
const sendSigninOTP = asynchandler(async (req, res) => {
    ...
    console.log('All users with this email:', allUsersWithEmail.map(u => ({ _id: u._id, username: u.username, password: u.password ? '[PRESENT]' : '[MISSING]', isEmailVerified: u.isEmailVerified, signinOTPLastSent: u.signinOTPLastSent, createdAt: u.createdAt })));
    // Use the user with the latest createdAt (most recent registration)
    const user = allUsersWithEmail.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))[0];
    if (!user) {
    ...
    console.log('User found:', {
        ...
        hasRealPassword: user.password && !user.password.startsWith('temp_password_'),
        hasRealUsername: !user.username.startsWith('temp_'),
        isFullyRegistered: user.isEmailVerified && 
            user.password && 
    ...
    const isFullyRegistered = user.isEmailVerified && 
        user.password && 
        !user.password.startsWith('temp_password_') &&
        !user.username.startsWith('temp_');
    ...
    try {
        ...
        return res.status(200).json(
            new ApiResponse(200, 'Signin OTP sent successfully', { email })
        );
    } catch (error) {
...
const verifySigninOTP = asynchandler(async (req, res) => {
    const { email, otp } = req.body;
    console.log('Signin OTP verification request:', { email, otp, bodyData: req.body });
    ...
    const user = allUsersWithEmail.sort((a, b) => new Date(b.signinOTPLastSent || 0) - new Date(a.signinOTPLastSent || 0))[0];
    ...
    console.log('User signin OTP data:', {
        ...
        isExpired: user.signinOTPExpiry ? new Date() > new Date(user.signinOTPExpiry) : 'NO_EXPIRY'
    });
    ...
    return res
        ...
        .json(new ApiResponse(200, 'Signin successful', {
            user: userResponse,
            accessToken,
            refreshToken
        }));
